...
(START)
    @SCREEN   // starting conditions, set address to first pixel
    D = A
    @addr
    M = D
...
(BLACK)	
    ...
    D = M
    @CHECK
    D;JGT  // jump back to CHECK if pixels remain

    0;JMP   // otherwise, keep looping
...
(BLANK)
    ...
    D = M
    @CHECK
    D;JGT   // loop back to CHECK if pixels remain
